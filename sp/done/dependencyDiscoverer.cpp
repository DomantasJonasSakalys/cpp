...
struct table {
    ...
    public:
        ...
        bool add (std::string name, std::list<std::string> *ll){
        ...
        void insert (std::string obj, std::string file){
            std::unique_lock<std::mutex> lock(m);
          
            // 3a. insert mapping from file.o to file.ext
            theTable.insert( { obj, { file } } );
    
            // 3b. insert mapping from file.ext to empty list
            theTable.insert( { file, { } } );
        }

        std::list<std::string>* find (std::string name){
            std::unique_lock<std::mutex> lock(m);
...
struct workQueue {
    ...
    public:
        ...
        std::string pop_front_and_front() {
            std::unique_lock<std::mutex> lock(m);
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (theTable.add(name, ll) == 0) workQ.push_back( name );
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = theTable.find(name);
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    int threadNum = 1;//std::thread::hardware_concurrency();
    std::vector<std::thread> threads;
  ...
  // 4. for each file on the workQ
  while ( workQ.size() > 0 ) {
    std::string filename = workQ.pop_front_and_front();

    if (theTable.find(filename) == 0) {
    ...
    return 0;}); }
    for (int n = 0; n < threadNum; n++) { threads[n].join(); }
